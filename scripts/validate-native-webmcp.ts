...
try {
  ...
  const registrationViolation = await page.evaluate(async () => {
    ...
    for (let attempt = 0; attempt < 200; attempt += 1) {
      ...
      if (document.querySelector(".console-shell") !== null) return count !== 4;
      await new Promise((resolve) => setTimeout(resolve, 10));
    }
    throw new Error("The console did not render after native registration.");
  });
  assert(!registrationViolation, "The console rendered before all four native tools registered.");

  const registered = await tools(page);
  const names = registered.map(({ name }) => name).sort();
  assert(JSON.stringify(names) === JSON.stringify(["fleet_status", "scenario_current", "vehicle_get", "vehicle_rename"]), `Unexpected native tools: ${names.join(", ")}`);
  ...
  assert(cleanup.before === 4 && cleanup.after === 0, `Native cleanup was ${cleanup.before}→${cleanup.after}, expected 4→0.`);
  assert(errors.length === 0, `Browser errors: ${errors.join(" | ")}`);
  console.log(JSON.stringify({ browser: await browser.version(), cleanup: "4→0", errors: 0, registrationBeforeRender: true, schemas: 4, tools: names }));
} finally {
  await browser.close();
}
