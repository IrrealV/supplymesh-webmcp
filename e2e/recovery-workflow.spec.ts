...
test("full workflow is agent-driven around the human approval boundary", async ({ page }) => {
  ...
  await expect(page.getByText(/Agent capability enabled.*recovery_plan_execute/)).toBeVisible(); await expect.poll(() => toolNames(page)).toContain("recovery_plan_execute"); await executeTool(page, "recovery_plan_execute", { planId }); await expect(page.getByText("EXECUTED", { exact: true })).toBeVisible();
  await expect(page.locator("#recovery-map-summary")).toHaveAttribute("data-route-state", "applied"); await expect(page.getByText("RESOLVED", { exact: true })).toBeVisible(); await expect(page.getByText(/Agent capability enabled.*recovery_verify/)).toBeVisible();
...
test("approval enables agent execute and verify without human execution controls", async ({ page }) => {
  ...
  await executeTool(page, "recovery_plan_execute", { planId }); await expect.poll(() => toolNames(page)).toContain("recovery_verify"); await executeTool(page, "recovery_verify", { planId }); await expect(page.getByText("VERIFIED", { exact: true })).toBeVisible(); await expect(page.getByRole("button", { name: "Reset demo" })).toBeVisible();
});
...
test("tablet 900x900 keeps the human review usable", async ({ page }) => {
  ...
  const bounds = await panel.boundingBox(); expect(bounds).not.toBeNull(); expect(bounds!.x).toBeGreaterThanOrEqual(0); expect(bounds!.x + bounds!.width).toBeLessThanOrEqual(900); expect(await page.evaluate(() => document.body.scrollWidth)).toBeLessThanOrEqual(900);
});
