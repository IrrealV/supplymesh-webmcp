...
export function VehicleInspection({ isFollowing, locale, onClose, onRestoreFollow, onScenarioChange, onViewRoute, operations, scenario, vehicle }: InspectionProps) {
  ...
  if (usesTabletDialog) return <Dialog.Root modal={false} onOpenChange={(open) => { if (!open) requestClose(); }} open={tabletDrawerOpen}><Dialog.Portal><Dialog.Content aria-describedby={undefined} aria-label={copy.vehicleInspection} className="vehicle-inspection tablet-vehicle-drawer">{contents}</Dialog.Content></Dialog.Portal></Dialog.Root>;
