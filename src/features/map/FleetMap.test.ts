...
describe("FleetMap layers", () => {
  ...
  it("should focus and render comparison layers from the promoted operation projection", () => {
    const mapSource = readFileSync("src/features/map/FleetMap.tsx", "utf8");
    const recoverySource = readFileSync("src/features/recovery-comparison/RecoveryComparisonLayers.tsx", "utf8");

    expect(mapSource).toContain('target.kind === "comparison"');
    expect(mapSource).toContain("<RecoveryComparisonLayers");
    expect(recoverySource).toContain("model.incident.position");
    expect(recoverySource).not.toContain("risk.geometry");
  });
