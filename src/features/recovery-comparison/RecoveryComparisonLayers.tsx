...
export function RecoveryComparisonLayers({ comparison, locale, model, onIncidentSelect, showUnit = false }: { comparison: boolean; locale: Locale; model: Unit211RecoveryComparisonModel; onIncidentSelect?(vehicleId: string): void; showUnit?: boolean }) {
  ...
  return <Fragment><Pane name="recovery-exclusion" style={{ zIndex: 420 }} /><Pane name="recovery-routes" style={{ zIndex: 430 }} /><Pane name="recovery-markers" style={{ zIndex: 630 }} />
    {comparison && <><Polygon pane="recovery-exclusion" pathOptions={{ className: "recovery-exclusion", color: "#7c3aed", dashArray: "7 5", fillColor: "#8b5cf6", fillOpacity: .2, weight: 2.5 }} positions={model.incident.exclusionCoordinates.map(latLng)} /><Polyline pane="recovery-routes" pathOptions={{ className: "recovery-current-route", color: "#d91929", dashArray: "10 8", opacity: .88, weight: 4 }} positions={model.current.coordinates.map(latLng)} /><Polyline pane="recovery-routes" pathOptions={{ className: "recovery-alternative-route", color: "#079455", opacity: .95, weight: 4.5 }} positions={model.alternative.coordinates.map(latLng)} /></>}
